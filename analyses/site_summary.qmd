...
data <- read_tsv("../outputs/taxonomy_v5.tsv") %>%
  ...
  left_join(read_tsv("../outputs/traits_v5.tsv"))
...
#| warning: false
data %>%
  select(genus_cleaned, species_cleaned, trait, species_trait_value_raw) %>%
  na.omit() %>%
  ...
  ggplot(aes(species_trait_value_raw)) +
  geom_histogram() +
...
data %>%
  ...
  summarise(species_trait_value_raw = mean(species_trait_value_raw)) %>%
  group_by(trait) %>%
  summarise(count = n(), percent = n() / n_tot * 100) %>%
  kable(caption = "Trait coverage across site species", digits = 2)
```

However, as the count revealed a lot of missing data, we used missing data imputation with random forest using the `missForest` R package to further impute missing trait values [@Gendre2024]. We first aggregated data at the genus level with the median trait value to inform the genus or species missing trait data. We obtained a normalised root mean square error for imputation of NRMSE=1.03. We obtained the PCA below:
...
t <- data %>% 
  select(genus_cleaned, trait, genus_trait_value_imputed) %>%
  na.omit() %>% 
    unique() %>% 
    pivot_wider(
      names_from = trait,
      values_from = genus_trait_value_imputed
    ) %>%
    select(-genus_cleaned) %>% 
    mutate(SLA = log(SLA), LA = log(LA)) %>% 
      na.omit()
autoplot(
  princomp(t, cor = TRUE),
  ...
  scale_fill_viridis_d("Genus density")
```
